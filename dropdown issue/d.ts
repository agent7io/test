...
/* ═══════════════════════════════════════════════════════════════════════════
 * SMART SELECT — REWRITTEN. Why some dropdowns worked and some didn't:
 *
 * ROOT CAUSE 1 — MENUS APPENDED TO <body>:
 *   bootstrap-select with data-container="body", Select2 (its default
 *   dropdownParent), and jQuery UI selectmenu all render their OPEN menu
 *   OUTSIDE the widget wrapper — directly under <body>. The old code searched
 *   for options INSIDE the wrapper (`wrapperEl.$(".dropdown-menu li a...")`),
 *   found nothing, and silently gave up. → Every "some dropdowns don't work"
 *   case where the menu detaches to body failed. All option lookups now run at
 *   DOCUMENT level (ctx.locator), which finds the menu wherever it renders.
 *
 * ROOT CAUSE 2 — jQuery UI selectmenu was NOT handled at all:
 *   It hides the native <select> and renders a sibling
 *   <span class="ui-selectmenu-button">. Setting el.value + trigger('change')
 *   updates the form data BUT the visible button label only refreshes via
 *   $el.selectmenu('refresh') — which was never called — and there was no UI
 *   fallback for dynamically-built option lists. Both are added now.
 *
 * ROOT CAUSE 3 — bootstrap-select (BS4/5) option markup:
 *   The option text lives in <a class="dropdown-item"><span class="text">Delhi
 *   </span></a>. Exact-matching with `li a:text-is("Delhi")` fails whenever the
 *   anchor also contains the check-mark <span> markup / extra whitespace, so
 *   the exact selector never hit and only the substring fallback ran — which
 *   can click "New Delhi" when you asked for "Delhi". We now exact-match on
 *   the inner `.text` span first.
 *
 * ROOT CAUSE 4 — Bootstrap 3 open-menu class:
 *   BS3 selectpicker marks the open widget with `.bootstrap-select.open`
 *   (there is NO `.dropdown-menu.show` in BS3). The old selectors only looked
 *   for `.dropdown-menu.show`, so every BS3 page failed. Both are covered now.
 *
 * ROOT CAUSE 5 — Select2 with AJAX/lazy options:
 *   The <option> elements don't exist in the native <select> until the
 *   dropdown opens and the search returns. Direct value assignment can never
 *   match, and there was no Select2 UI-drive fallback (only bootstrap-select
 *   had one) — so those dropdowns always failed. A full Select2 drive
 *   (open → type in .select2-search__field → wait for results → click) is
 *   added, including waiting out the "Searching…" placeholder.
 * ═══════════════════════════════════════════════════════════════════════════ */

/**
 * Click the first VISIBLE element among a list of candidate selectors.
 * Lookups run on ctx (Page or Frame) so body-appended menus are found in the
 * correct document — including when the widget lives inside an iframe.
 */
...
/**
 * UI drive for bootstrap-select (selectpicker): open toggle → live-search →
 * click option. Menu is looked up at DOCUMENT level to support
 * data-container="body", and both BS3 (.bootstrap-select.open) and BS4/5
 * (.dropdown-menu.show) open-menu markup are handled.
 */
async function driveBootstrapSelect(
  ctx: any,
  pg: any,
  handle: any,
  value: string,
  ...
  const wrapperHandle = await handle
    ...
    .catch(() => null);
  const wrapper =
    wrapperHandle && wrapperHandle.asElement ? wrapperHandle.asElement() : null;
  if (!wrapper) return false;

  try {
    // Open the dropdown
    const toggle = await wrapper.$("button.dropdown-toggle, .dropdown-toggle");
    if (toggle) await toggle.click({ timeout: 5000 });

    // Live-search box — may be inside the wrapper OR in a body-appended menu,
    // so look it up at document level.
    try {
      const search = ctx.locator(".bs-searchbox input").first();
      if (await search.isVisible().catch(() => false)) {
        await search.fill(value, { timeout: 3000 });
        await pg.waitForTimeout(400); // let live-search filter the list
      }
    } catch {}

    const picked = await clickFirstVisibleOption(ctx, [
      // BS4/5 markup: text lives in span.text — EXACT match there first so
      // "Delhi" never clicks "New Delhi"
      `.dropdown-menu.show a .text:text-is("${value}")`,
      `.bootstrap-select.open .dropdown-menu li a .text:text-is("${value}")`,
      // Older markup without span.text — exact match on the anchor
      `.dropdown-menu.show li a:text-is("${value}")`,
      `.bootstrap-select.open .dropdown-menu li a:text-is("${value}")`,
      // Substring fallback (both BS versions)
      `.dropdown-menu.show li a:has-text("${value}")`,
      `.bootstrap-select.open .dropdown-menu li a:has-text("${value}")`,
    ]);

    if (!picked) {
      // Close the open menu so it doesn't block later steps
      try {
...
/**
 * UI drive for Select2 (v4 and v3): open the rendered container (sibling of the
 * hidden <select>) → type in the search field (which lives in the
 * body-appended dropdown) → wait for AJAX results → click the option.
 */
async function driveSelect2(
  ctx: any,
  pg: any,
  handle: any,
  value: string,
  ...
  try {
    const openHandle = await handle
      .evaluateHandle((el: any) => {
        const sib = el.nextElementSibling;
        if (sib && /select2/i.test(sib.className || "")) {
          return sib.querySelector(".select2-selection") || sib;
        }
        return null;
      })
      .catch(() => null);
    const openEl =
      openHandle && openHandle.asElement ? openHandle.asElement() : null;
    if (!openEl) return false;

    await openEl.click({ timeout: 5000 });

    // The search field renders inside the body-appended dropdown → document level
    try {
      const search = ctx.locator("input.select2-search__field").first();
      if (await search.isVisible().catch(() => false)) {
        await search.fill(value, { timeout: 3000 });
        // AJAX-loaded options: wait for the "Searching…" placeholder to clear
        try {
          await ctx.waitForFunction(
            () =>
              !document.querySelector(
                ".select2-results__option.loading-results, .select2-results__option--load-more, .select2-searching",
              ),
            { timeout: 8000 },
          );
        } catch {}
    ...
    const picked = await clickFirstVisibleOption(ctx, [
      `.select2-results__option:text-is("${value}")`,
      `.select2-results__option:has-text("${value}")`,
      // Select2 v3 legacy markup
      `.select2-results .select2-result-label:text-is("${value}")`,
      `.select2-results .select2-result-label:has-text("${value}")`,
    ]);
...
/**
 * UI drive for jQuery UI selectmenu: click the generated button (sibling
 * span.ui-selectmenu-button, or #{id}-button) → click the item in the
 * body-appended .ui-selectmenu-menu. Handles both jQuery UI 1.12+
 * (.ui-menu-item-wrapper divs) and 1.11 (li.ui-menu-item > a) markup.
 */
async function driveJqueryUiSelectmenu(
  ctx: any,
  pg: any,
  handle: any,
  value: string,
  ...
  try {
    const btnHandle = await handle
      .evaluateHandle((el: any) => {
        const sib = el.nextElementSibling;
        if (sib && /ui-selectmenu-button/i.test(sib.className || ""))
          return sib;
        // jQuery UI generates the button with id "{selectId}-button"
        if (el.id) return document.getElementById(el.id + "-button");
        return null;
      })
      .catch(() => null);
    const btn = btnHandle && btnHandle.asElement ? btnHandle.asElement() : null;
    if (!btn) return false;
    ...
    const picked = await clickFirstVisibleOption(ctx, [
      // jQuery UI 1.12+
      `.ui-selectmenu-open .ui-menu-item-wrapper:text-is("${value}")`,
      `.ui-selectmenu-open .ui-menu-item-wrapper:has-text("${value}")`,
      // jQuery UI 1.11 and earlier
      `.ui-selectmenu-open li.ui-menu-item:text-is("${value}")`,
      `.ui-selectmenu-open li.ui-menu-item:has-text("${value}")`,
      // Menu open but without the ui-selectmenu-open marker (older themes)
      `ul.ui-menu:visible .ui-menu-item-wrapper:text-is("${value}")`,
      `ul.ui-menu:visible li.ui-menu-item:has-text("${value}")`,
    ]);
...
/**
 * UI drive for Chosen: click the sibling .chosen-container → type in its
 * search box → click the matching result.
 */
async function driveChosen(
  ctx: any,
  pg: any,
  handle: any,
  value: string,
  ...
  try {
    const cHandle = await handle
      .evaluateHandle((el: any) => {
        const sib = el.nextElementSibling;
        return sib && /chosen-container/i.test(sib.className || "")
          ? sib
          : null;
      })
      .catch(() => null);
    const c = cHandle && cHandle.asElement ? cHandle.asElement() : null;
    if (!c) return false;
    ...
    const picked = await clickFirstVisibleOption(ctx, [
      `.chosen-results li.active-result:text-is("${value}")`,
      `.chosen-results li.active-result:has-text("${value}")`,
...
async function smartSelect(
    ...
    for (const el of els) {
      const activeHere = await el
        .evaluate((node: any) => {
          ...
          // Select2 v4 / Chosen / jQuery UI selectmenu render the container as a
          // SIBLING after the select
          const sib = node.nextElementSibling;
          if (
            sib &&
            /select2|bootstrap-select|chosen|ui-selectmenu/i.test(
              sib.className || "",
            ) &&
            visible(sib)
          )
  ...
  if (tagName === "select") {
    ...
    const applied = await handle.evaluate((el: any, val: string) => {
      ...
      if (jq) {
        try {
          ...
          // FIX: jQuery UI selectmenu — without this refresh the value IS set on
          // the form but the visible button keeps showing the OLD label, which
          // looked like "the select didn't work"
          if (typeof $el.selectmenu === "function") {
            try {
    ...
    const widget = await handle
      .evaluate((el: any) => {
        if (el.closest && el.closest(".bootstrap-select"))
          return "bootstrap-select";
        const sib = el.nextElementSibling;
        const cls = (sib && sib.className) || "";
        if (/select2/i.test(cls)) return "select2";
        if (/ui-selectmenu/i.test(cls)) return "selectmenu";
        if (/chosen-container/i.test(cls)) return "chosen";
        // selectmenu button may be located by generated id instead of sibling
        if (el.id && document.getElementById(el.id + "-button"))
          return "selectmenu";
        return "";
      })
      .catch(() => "");
    ...
    let picked = false;
    if (widget === "bootstrap-select") {
      picked = await driveBootstrapSelect(ctx, pg, handle, value);
    } else if (widget === "select2") {
      picked = await driveSelect2(ctx, pg, handle, value);
    } else if (widget === "selectmenu") {
      picked = await driveJqueryUiSelectmenu(ctx, pg, handle, value);
    } else if (widget === "chosen") {
      picked = await driveChosen(ctx, pg, handle, value);
    } else {
      // Unknown enhancement — try each drive in order of likelihood
      picked =
        (await driveBootstrapSelect(ctx, pg, handle, value)) ||
        (await driveSelect2(ctx, pg, handle, value)) ||
        (await driveJqueryUiSelectmenu(ctx, pg, handle, value)) ||
        (await driveChosen(ctx, pg, handle, value));
    }
  ...
  // Case 4: custom (non-<select>) dropdown widget → open it, search, and click
  // the matching option. Exact text match FIRST, and DOCUMENT-level lookups so
  // body-appended menus are found.
  try {
  ...
  const pickedGeneric = await clickFirstVisibleOption(ctx, [
    // Exact matches first — never let "Delhi" click "New Delhi"
    `.select2-results__option:text-is("${value}")`,
    `.dropdown-menu.show a .text:text-is("${value}")`,
    `.dropdown-menu.show li a:text-is("${value}")`,
    `.bootstrap-select.open .dropdown-menu li a:text-is("${value}")`,
